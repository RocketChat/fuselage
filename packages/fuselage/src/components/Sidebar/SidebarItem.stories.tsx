import type { Meta, StoryObj } from '@storybook/react-webpack5';

import { Avatar } from '../Avatar';

import {
  SidebarItem,
  SidebarItemAvatar,
  SidebarItemContent,
  SidebarItemAction,
  SidebarItemActions,
  SidebarItemContainer,
  SidebarItemIcon,
  SidebarItemSubtitle,
  SidebarItemTitle,
  SidebarItemWrapper,
} from './Item';

export default {
  title: 'Sidebar/Item',
  component: SidebarItem,
  subcomponents: {
    SidebarItemAvatar,
    SidebarItemContent,
    SidebarItemContainer,
    SidebarItemIcon,
    SidebarItemSubtitle,
    SidebarItemTitle,
    SidebarItemWrapper,
  },
  argTypes: {
    selected: {
      control: 'boolean',
      description: 'Marks the item as the currently selected sidebar entry.',
    },
    highlighted: {
      control: 'boolean',
      description:
        'Highlights the item, e.g. to indicate an active call or mention.',
    },
    clickable: {
      control: 'boolean',
      description:
        'Renders the item with clickable styling even when `is` is not `a`.',
    },
    featured: {
      control: 'boolean',
      description: 'Applies the featured item styling.',
    },
    is: {
      control: 'text',
      description: 'Underlying element or component rendered as the item.',
    },
    children: {
      control: false,
      description: 'Content of the sidebar item.',
    },
  },
} satisfies Meta<typeof SidebarItem>;

type Story = StoryObj<typeof SidebarItem>;

const sidebarItemActions = (
  <SidebarItemActions>
    <SidebarItemAction aria-label='accept' secondary success icon='phone' />
    <SidebarItemAction
      aria-label='cancel'
      secondary
      danger
      icon='circle-cross'
    />
    <SidebarItemAction aria-label='delete' secondary info icon='trash' />
    <SidebarItemAction aria-label='accept' secondary icon='phone' />
  </SidebarItemActions>
);

export const Default: Story = {
  render: () => (
    <>
      <SidebarItem>
        <SidebarItemAvatar>
          <Avatar
            size='x16'
            url={
              'data:image/jpeg;base64,'
            }
          />
        </SidebarItemAvatar>
        <SidebarItemContent>
          <SidebarItemWrapper>
            <SidebarItemIcon icon='lock' />
            <SidebarItemTitle>
              ddsfoasdhfiuashd ousadhfiuasdhfiuashd soduhfiuasdhfia
              ishdfiuashdfiuas iuashdfiausdh
            </SidebarItemTitle>
          </SidebarItemWrapper>
        </SidebarItemContent>
        <SidebarItemContainer>{sidebarItemActions}</SidebarItemContainer>
      </SidebarItem>

      <SidebarItem highlighted is='a'>
        <SidebarItemAvatar>
          <Avatar
            size='x28'
            url={
              'data:image/jpeg;base64,'
            }
          />
        </SidebarItemAvatar>
        <SidebarItemContent>
          <SidebarItemWrapper>
            <SidebarItemIcon icon='lock' />
            <SidebarItemTitle>highlighted</SidebarItemTitle>
          </SidebarItemWrapper>
        </SidebarItemContent>
        <SidebarItemContainer />
      </SidebarItem>
      <SidebarItem selected is='a'>
        <SidebarItemAvatar>
          <Avatar
            size='x28'
            url={
              'data:image/jpeg;base64,'
            }
          />
        </SidebarItemAvatar>
        <SidebarItemContent>
          <SidebarItemWrapper>
            <SidebarItemIcon icon='lock' />
            <SidebarItemTitle>
              ddsfoasdhfiuashd ousadhfiuasdhfiuashd soduhfiuasdhfia
              ishdfiuashdfiuas iuashdfiausdh
            </SidebarItemTitle>
          </SidebarItemWrapper>
        </SidebarItemContent>
        <SidebarItemContainer>{sidebarItemActions}</SidebarItemContainer>
      </SidebarItem>
      <SidebarItem is='a'>
        <SidebarItemAvatar>
          <Avatar
            size='x36'
            url={
              'data:image/jpeg;base64,'
            }
          />
        </SidebarItemAvatar>
        <SidebarItemContent>
          <SidebarItemContent>
            <SidebarItemWrapper>
              <SidebarItemIcon icon='lock' />
              <SidebarItemTitle>
                ddsfoasdhfiuashd ousadhfiuasdhfiuashd soduhfiuasdhfia
                ishdfiuashdfiuas iuashdfiausdh
              </SidebarItemTitle>
            </SidebarItemWrapper>
          </SidebarItemContent>
          <SidebarItemContent>
            <SidebarItemWrapper>
              <SidebarItemSubtitle>
                ddsfoasdhfiuashd ousadhfiuasdhfiuashd soduhfiuasdhfia
                ishdfiuashdfiuas iuashdfiausdh
              </SidebarItemSubtitle>
            </SidebarItemWrapper>
          </SidebarItemContent>
        </SidebarItemContent>
        <SidebarItemContainer>
          <SidebarItemContainer>{sidebarItemActions}</SidebarItemContainer>
        </SidebarItemContainer>
      </SidebarItem>
    </>
  ),
};
