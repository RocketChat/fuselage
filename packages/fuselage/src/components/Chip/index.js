...
Chip.Wrapper = ({ children, width, alignItems = 'center', wrap = 'wrap', ...props }) => <Flex.Container alignItems={alignItems} wrap={wrap}><Wrapper {...props}> {children.map((children, i) => <Flex.Item key={i} shrink={1}><Margins all={4}> {children}</Margins></Flex.Item>)} </Wrapper></Flex.Container>;
