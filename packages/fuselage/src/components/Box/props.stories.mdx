...
import { flattenChildren, Box, Button } from '../..';
...
<Preview>
  <Story
    ...
    decorators={[
      (fn) => <Box display='flex' flexWrap='wrap' alignItems='center'>
        {flattenChildren(fn().props.children).map((child) => <Box key={child.key} position='relative' bg='neutral-200' m='x16' size='x64'>
...
<Preview>
  <Story name='With rich content'>
    <Box withRichContent>
      ...
      <p><em>This text has emphasis</em></p>
