...
import { Box, Flex, MarginsWrapper, Margins } from '../Box';
...
const Container = ({ children, ...props }) => <Box {...props} is='label' className='rcx-select'>{console.log(props, children)}{React.Children.map(children, (c, i) => <Margins key={i} inline={4}>{c}</Margins>)}</Box>;
