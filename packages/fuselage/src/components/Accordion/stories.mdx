import { Meta, Preview, Props, Story } from '@storybook/addon-docs/blocks';
import LinkTo from '@storybook/addon-links/react';

import { Accordion, Box } from '../..';

<Meta title='Containers/Accordion' parameters={{ jest: ['Accordion/spec'] }} />

# Accordion

An <LinkTo kind='Containers|Accordion' story='Default Story'>`Accordion`</LinkTo> allows users to toggle the display of
sections of content.

<Preview>
  <Story name='Default'>
    <Accordion>
      <Accordion.Item title='Item #1' defaultExpanded>
        <Box textStyle='p1' color='default' marginBlockEnd='x16'>Content #1</Box>
      </Accordion.Item>
      <Accordion.Item title='Item #2'>
        <Box textStyle='p1' color='default' marginBlockEnd='x16'>Content #2</Box>
      </Accordion.Item>
      <Accordion.Item title='Item #3'>
        <Box textStyle='p1' color='default' marginBlockEnd='x16'>Content #3</Box>
      </Accordion.Item>
    </Accordion>
  </Story>
</Preview>

<Props of={Accordion} />
