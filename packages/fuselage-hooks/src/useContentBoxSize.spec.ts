...
describe('useContentBoxSize hook', () => {
  beforeAll(() => {
    jest.useFakeTimers();
  ...
  it('gets the observed element size', async () => {
    ...
    const { contentBoxSize: expectedSize } = createBoxSizes(initialStyle);
  ...
  it('gets the observed element size after resize', async () => {
    ...
    const { contentBoxSize: expectedSizeA } = createBoxSizes(initialStyle);

    const newStyle = createRandomStyle();

    const { contentBoxSize: expectedSizeB } = createBoxSizes(newStyle);
