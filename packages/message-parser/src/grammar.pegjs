 ...
 * ```
 */
Code = "```" language:Code_language? EndOfLine lines:Code_line+ EndOfLine "```" { return code(lines, language); }
 ...
 *
*/
Heading = count:HeadingStart [ \t]+ text:Heading_chunk { return heading([text], count); }
...
task
  = "- [" flag:taskFlag "]" [ \t]+ text:Inline { return task(text, flag); }
...
UnorderedList_itemContent_item
  = InlineItem
  / !"*" @Any
...
italic_Content = text:italic_Content_item+ { return reducePlainTexts(text); }
...
/* Bold */
Bold = [\x2A] [\x2A] @Bold_Content [\x2A] [\x2A] / [\x2A] @Bold_Content [\x2A]

Bold_Content = text:Bold_Content_Item+ { return bold(reducePlainTexts(text)); }

Bold_Content_Item = References / Italic / Strikethrough / Line / AnyBold

/* Strike */
Strikethrough = [\x7E] [\x7E] @Strikethrough_Content [\x7E] [\x7E] / [\x7E] @Strikethrough_Content [\x7E]

Strikethrough_Content = text:(References / Italic / Bold / Line / AnyStrike)+ {
...
UserMention
  ...
  / "@"+ user:$(utf8_names_validation ([:@] utf8_names_validation)?) {
 ...
 *
 */
BigEmoji = _ es:(@(Emoji / emoticon) _) |1..3| { return [bigEmoji(es)]; }

emoji_shortCode_name = $[0-9a-zA-Z-_+.]+

Emoji = Emoji_shortCode / ch:unicodeEmoji { return emojiUnicode(ch); }

Emoji_shortCode = ":" shortCode:emoji_shortCode_name ":" { return emoji(shortCode); }
